...
mtcnn = MTCNN(image_size=160, margin=40, keep_all=False, thresholds=[0.4, 0.5, 0.6], min_face_size=15, device='cuda' if torch.cuda.is_available() else 'cpu')

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Face tracking buffer to stabilize detections
face_buffer = []
BUFFER_SIZE = 5  # Number of frames to average detections over
...
def run_webcam(model_path="face_model.pkl"):
    ...
    while True:
        ...
        if boxes is not None:
            ...
            # Store detected boxes in a buffer for stabilization
            if len(face_buffer) >= BUFFER_SIZE:
                face_buffer.pop(0)  # Remove oldest frame detection
            face_buffer.append(boxes)
            
            # Compute average box over buffer for smoother detection
            avg_boxes = np.mean(np.array(face_buffer), axis=0)
            
            for box in avg_boxes:
